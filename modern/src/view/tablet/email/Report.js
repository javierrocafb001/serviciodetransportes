...
Ext.Msg.prompt("Numero de Dias", "Ingresa el Numero de dias Atras, del Reporte que deseas Ver, por ejemplo 0 esto es igual a hoy:", function(btnText, sInput){
                if(btnText === 'ok'){
                if(sInput <= 365) {
                Ext.data.StoreManager.lookup('reportperunit').getProxy().extraParams = { store: 'reportperunit', days: sInput, 
		unitnumber: passedvalue, reportdate: Ext.Date.format(new Date(), 'Y/m/d')  }; 
