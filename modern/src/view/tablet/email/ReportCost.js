Ext.define('Admin.view.tablet.email.ReportCost', {
	...
	painted: function(){
		...
		Ext.Msg.prompt("Numero de Dias", "Ingresa el Numero de dias Atras, del Reporte que deseas Ver, por ejemplo 0 esto es igual a hoy:", function(btnText, sInput){
                if(btnText === 'ok'){
		if(sInput <= 365){	
                Ext.data.StoreManager.lookup('reportpercost').getProxy().extraParams = { store: 'reportpercost', days: sInput, 
		reportdate: Ext.Date.format(new Date(), 'Y/m/d'), price: localStorage.getItem('priceofroute'), 
